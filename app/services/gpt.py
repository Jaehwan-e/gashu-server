...
1. 대화 기록을 포함해 현재 상태를 "set_dep", "set_dest", "main", "error" 중 하나로 분류해줘.
    - "set_dep": 사용자가 출발지를 설정하는 단계
    - "set_dest": 사용자가 목적지를 설정하는 단계
    - "main": 출발지와 목적지가 결정된 상태로 버스 정보를 안내하는 단계
    - "error": 사용자의 메시지가 버스 정보 안내와 무관한 경우
2. 사용자 메시지에서 **출발지(dep)**와 **목적지(dest)**를 추출해줘. 문장에 명시되지 않았다면 null로 설정해. 출발지가 현재 위치, 지금 위치 등 비슷한 말이라면 dep를 "현재 위치"로 설정해
...
5. 사용자의 메시지가 **버스 정보 안내와 무관한 일반적인 발화**(예: 잡담, 다른 주제)라면 "error": true로 설정해. 대화 기록에 같은 내용이 3회 이상 반복되는 경우도 error로 설정해.
그 외에는 false로 설정해.
